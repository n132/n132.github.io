...
```
In the if statement, there is a one slot off, where should be corrected to `pgoff >= sbuf->pagecount`. Thus, if `pgoff == 0x80`, we are able to operate the page struct at the slot out of bound. To exploit the bug, we should figure out 
...
If we mmap pages in the user space without operating these pages, these pages are not really allocated to make allocation fatser. When we try to read/write these pages, `vm_fault` is triggered and corresponding handler will be executed. They allocate pages then user space are able to perform further operations. 

`struct vm_fault *vmf` is the parameter passed to the `vm_fault` handler. The `pgoff` element is the page index between the target page and the start of the vma. In this challenge, it's the operated page and the start of file (check `dev_mmap`). Because of the check of max `mmap` page, we need at least set the last parameter of `mmap` to 0x1000 if we allocate 0x80 pages or we can allocate 0x1 page with setting the offset to 0x80000.
...
In this challenge, we have oob access of a page list. So we have to put/fake a page struct at the oob slot. It's complex to fake a page struct at the oob slot since we didn't leak any address so use some struct to put a page struct at the oob slot could be easier. 
...
As we see in the structure, a pointer of page struct is at the start of the struct which will be at the oob slot. To allocate `pipe_buffer` struct next to the target object (0x400), we have to use `pipe_fcntl` to resize the size. 
...
By spraying pipe_buffer arraies based on the code above, I place a page struct at the oob slot. 

# 0x04 Primitives

This oob is actually an UAF: 
- Place a page struct at the oob slot
- Free the pages where the page struct points to
- Operate the oob slot

Luckily, closing the pipes will not clean the metadata on the the `pipe_buffer` struct so we can still operate the page struct. Considering the freed page is collected, cross cache techniques could be applied. I used to do control flow hijacking after I have UAF but I noticed @lotus used a simpler solution:
...
This solution is simpler which doesn't requires control flow hijacking.
...
I checked the official write up and found they didn't set the offset of mmap but using mremap to bypass the check in the function `dev_mmap`.
