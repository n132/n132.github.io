...
I spent another 3 hours to review the malloc's source code to find a way to return the "NULL". In my memory, malloc would return a negtive number on fail and return the address of chunk when successing. And this 3 hours proves that, we have no way to return a 0 by construct a bad child-api call.
...
3. Send some api call to write/read arbitrary address to hijack the parent process
...
By setting the second paremeter, we can limit different types of resource of a process, such as CPU time/core, Address Space. And there are some of them are memory related, including
...
I also noticed that not only the parent and child process could limit each other, but a normal user's process could influence other process in user space. A very interesting feature!
...
4. Back to `child_note` and trigger the vulnerability to change the size to `-1`. My method is a little verbose. I trigger the `brk` by reallocate the top chunk.
...
This challenge is interesting, cuz before the first boold was got by `Shellphish` I can't believe it's solvale and I did check the attachment so many times, lol. Yeah, after solving this, I think there are enough clues left in the challenge, the blacklist and the meaningless `-1` for an unsigned int. This challenge is not complex but educational and it's a good challenge in my view.

Another fun fact, this type of vulnerabilities are not likely found by the fuzzer. Because the fuzzer can't even trigger the branch! I beleive there are so many this type of vulnerabilities in real world. And this challenge's solution introduces a new way to implement some "impossible" attackings.
